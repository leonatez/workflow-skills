...
**If `DEPLOY_MODE` is already set to `coolify-cloudflare` or `none`**: machine is configured. Skip to Phase 0.

**If `DEPLOY_MODE` is missing or still has a placeholder value**: ask the user one question:

> "Is this machine a deployment server (Coolify + Cloudflare Tunnel), or is it dev-only (local uvicorn + npm run dev, no Docker)?"
...
- **Coolify + Cloudflare** → ask the following, one at a time, then write the full file:

  1. Coolify base URL (e.g. `http://localhost:8000`, or the tunnel URL like `https://admin.yourdomain.com`)
  2. Coolify API token (`ID|secret`) — minted at Coolify → Keys & Tokens → API tokens, with scopes `read`, `read:sensitive`, `write`, `deploy`
  3. Cloudflare Tunnel ID — tell user to run `cloudflared tunnel list` if unsure
  4. Root domain (e.g. `enginxlabs.com`)
  5. Path to cloudflared config file (default: `/etc/cloudflared/config.yml`)
  6. Short description of this machine (for NOTES field)

  Then write `~/.claude/machine-config.md`:
  ```
  DEPLOY_MODE=coolify-cloudflare
  COOLIFY_URL=[answer 1]
  COOLIFY_API_TOKEN=[answer 2]
  COOLIFY_SERVER_UUID=
  GITHUB_APP_UUID=
  TUNNEL_ID=[answer 3]
  TUNNEL_CNAME_TARGET=[answer 3].cfargotunnel.com
  CLOUDFLARE_CONFIG_FILE=[answer 5]
  DOMAIN=[answer 4]
  NOTES=[answer 6]
  ```

  `COOLIFY_SERVER_UUID` and `GITHUB_APP_UUID` can be left blank — the DevOps Agent auto-detects them. Note that the GitHub App must be connected once in Coolify (Sources → GitHub App) before any private repo can be deployed.
...
**8. File storage (MinIO) — ask this before any other work starts**

Ask: *"Will this app accept file uploads — images, documents, or any other files?"*

If **no**: skip this item. Do not set up MinIO.

If **yes**: the bucket and service account must exist before Backend PM writes a single line of upload code. Handle it now, based on the current machine's `DEPLOY_MODE`:

- **`DEPLOY_MODE=coolify-cloudflare`** (running on the miniPC): delegate to DevOps Agent to run Step 0.5. Wait for it to return credentials, then write them to `PROJECT_CONFIG.md`.

- **`DEPLOY_MODE=none`** (running on a dev PC): DevOps Agent cannot run Step 0.5 here. Guide the user to set it up manually — it takes two minutes in the browser:
  1. Open `https://minio.enginxlabs.com` — login credentials are in `~/.claude/machine-config.md` (MINIO_ROOT_USER / MINIO_ROOT_PASSWORD) on the miniPC, or ask the user
  2. Buckets → Create bucket → name: `[project-name]-uploads` → access: Public
  3. Identity → Service Accounts → Create service account → name: `[project-name]-svc` → copy the Access Key and Secret Key
  4. Ask the user to paste the Access Key and Secret Key back into this conversation

In both cases, once credentials are in hand, write to `PROJECT_CONFIG.md` under a `## MinIO` section:
...
Then pass these four values to Backend PM as part of the project setup. The same credentials are used for local development and production — no environment switching.

**9. Deployment config (Coolify + Cloudflare)**
- GitHub repo URL + default branch (Coolify deploys directly from git — confirm the repo already collected in item 5)
- Backend app name (e.g. `myproject-api`) → will be served at `myproject-api.enginxlabs.com`
- Frontend app name (e.g. `myproject`) → will be served at `myproject.enginxlabs.com`
- Backend internal port — the port FastAPI/uvicorn listens on inside the container (default: `8000`)
...
- Cloudflare Zone ID for `enginxlabs.com` (find in Cloudflare dashboard → domain overview → right sidebar)

The Coolify API token is machine-scoped and already lives in `~/.claude/machine-config.md` — not collected per project. Store the Cloudflare API token in `.env` only — never in `PROJECT_CONFIG.md`. The DevOps Agent fills in the Coolify project/app UUIDs in `PROJECT_CONFIG.md` after the first deploy. Builds use Coolify's Nixpacks (no Dockerfiles), and push-to-main auto-deploys via Coolify's GitHub App (no GitHub Actions).
...
## MinIO (only if app handles file uploads)
MINIO_ENDPOINT=https://storage.enginxlabs.com
MINIO_ACCESS_KEY=[key]
MINIO_SECRET_KEY=[secret]
MINIO_BUCKET=[project-name]-uploads
...
- **Coolify dashboard:** https://admin.enginxlabs.com (API: http://localhost:8000/api/v1)
- **Backend app name:** [name] → https://[name].enginxlabs.com
- **Frontend app name:** [name] → https://[name].enginxlabs.com
- **Build pack:** nixpacks (no Dockerfiles)
- **Auto-deploy:** push to main → Coolify redeploys via GitHub App (no GitHub Actions)
- **Coolify project UUID:** [filled by DevOps Agent after first deploy]
- **Backend app UUID / Frontend app UUID:** [filled by DevOps Agent after first deploy]
- **Cloudflare Zone ID:** [zone-id]
- **Tunnel ID:** a27daddb-e6e9-49b8-8925-06a80210415f
- **Tunnel config file:** /etc/cloudflared/config.yml
- **Coolify API token:** stored in ~/.claude/machine-config.md (machine-scoped)
